...
AskUserQuestion:
  ...
  options:
    - "Yes (Claude/Sessions, Claude/Learnings, Claude/Tasks, Claude/Ideas)"
    - "Customize folders"
```
...
```bash
# Synchronous — fast file index
qmd update --collection "${qmd_collection}"

# Background — embedding generation (takes time)
...
```bash
obsidian vault="${vault_name}" create name="Claude Index" content="# Claude Code Notes\n\nAuto-generated by obsidian-sync." silent
```
