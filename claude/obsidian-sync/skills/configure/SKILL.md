---
name: configure
description: Configure obsidian-sync settings — vault name, vault path, qmd collection, and folder structure. Use when the user says "configure obsidian", "set vault path", "obsidian config", "change vault", "switch vault", or when another obsidian-sync skill reports that config is missing.
version: 0.2.0
user-invocable: true
...
- Default path: `${XDG_CONFIG_HOME:-~/.config}/obsidian-sync/config.yaml`
...
Read the existing config from `~/.config/obsidian-sync/config.yaml`.
...
```text
Configuration saved to ~/.config/obsidian-sync/config.yaml
